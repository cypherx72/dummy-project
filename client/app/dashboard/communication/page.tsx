...
function AnnouncementsTab() {
  ...
  async function onSubmit(values: AnnouncementFormValues) {
    ...
    console.log("Announcement:", values);
    toast.success("Announcement sent successfully", {
      description: `Sent to ${values.recipientType === "all" ? "all recipients" : "selected recipients"}`,
    });
    form.reset();
    setAttachments([]);
    setShowForm(false);
  ...
  function handleAttachment() {
    ...
    toast.info("Attachment added", { description: fileName });
  }
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-bold text-2xl">
                  {
                    MOCK_ANNOUNCEMENTS.filter((a) => a.priority === "urgent")
                      .length
                  }
                </p>
              </div>
                    ...
                    <div className="flex-1 space-y-1">
                      ...
                      <div className="flex flex-wrap items-center gap-3 pt-1 text-xs">
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {announcement.totalRecipients} ({readPercentage}%)
                        </span>
                      </div>
                    </div>
...
function MeetingsTab() {
  ...
  async function onSubmit(values: MeetingFormValues) {
    await new Promise((r) => setTimeout(r, 1000));
    console.log("Meeting slots:", values);
    toast.success("Meeting slots created successfully", {
      description: `${values.maxSlots} slot(s) created for ${format(values.date, "PPP")}`,
    });
    form.reset();
    setShowForm(false);
  }
  ...
  const availableSlots = MOCK_MEETINGS.filter((m) => m.status === "available");
  const bookedSlots = MOCK_MEETINGS.filter((m) => m.status === "booked");
