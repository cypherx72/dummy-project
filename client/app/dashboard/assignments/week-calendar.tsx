...
const annotations: Annotation[] = [
  ...
  { date: new Date(2026, 2, 15), label: "Launch day 🚀", color: "bg-green-500" },
];
      ...
      <Calendar
        ...
        components={{
          DayButton: ({ day, ...props }) => {
                ...
                {dayAnnotations.length > 0 && (
                  <span className="bottom-0.5 left-1/2 absolute flex gap-0.5 -translate-x-1/2">
                    {dayAnnotations.map((a, i) => (
                      <span key={i} className={`w-1.5 h-1.5 rounded-full ${a.color}`} />
                    ))}
                  </span>
                )}
              </button>
            ...
            return (
              <Tooltip>
                ...
                <TooltipContent>
                  {dayAnnotations.map((a, i) => (
                    <div key={i} className="flex flex-col justify-center items-center gap-1.5">
          ...
          PreviousMonthButton: (props) => (
            <button {...props} onClick={() => setCurrentDate((d) => subWeeks(d, 1))} className="p-1">
              <ChevronLeft className="w-4 h-4" />
            </button>
          ),
          NextMonthButton: (props) => (
            <button {...props} onClick={() => setCurrentDate((d) => addWeeks(d, 1))} className="p-1">
