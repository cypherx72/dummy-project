...
// ─── Types & helpers (extracted to _helpers/assignment-helpers.ts) ─────────────
...
import type {
  ...
  Course,
} from "../_helpers/assignment-helpers";
import {
  parseDate,
  getAssignmentStatus,
} from "../_helpers/assignment-helpers";
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-1">
            ...
            <span className="text-muted-foreground text-sm">
              {assignment.course.name} · {assignment.teacher.name}
            </span>
            {dueBadge && (
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-2.5 text-sm">
              <MetaRow label="Course" value={assignment.course.name} />
    ...
    if (live.length > 0) {
      setQuizzes(
        live.map((q: any) => ({
          ...
          teacher: q.createdById ?? "—",
          dueDate: q.dueDate ?? new Date().toISOString(),
          postedDate: q.createdAt ?? new Date().toISOString(),
          status: (q.isPublished ? "available" : "locked") as QuizStatus,
          durationMinutes: q.timeLimit ?? 30,
        ...
        <TabsContent value="assignments" className="mt-6">
          {fetchAssignmentsLoading ? (
            ...
            <AssignmentsTab
              ...
              onSelect={(a) => router.push(`/assignments/upload/${a.id}`)}
            />
          )}
        </TabsContent>
